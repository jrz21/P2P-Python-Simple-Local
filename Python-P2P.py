...
def connect_to_neighbor(host_ip, host_port):
    try:
        ...
        client_name = f"Vecino_{last_ip_number}"
...
def auto_connect_to_neighbor(neighbor_ip, neighbor_port, neighbor_count):
    try:
        ...
        if (neighbor_ip, neighbor_port) in connections:
            print(f"Ya estás conectado con {neighbor_ip}:{neighbor_port}.")
            return
...
while True:
    ...
    # Asignar un nombre único al cliente como "Vecino 1", "Vecino 2", etc.
    client_name = f"Vecino {len(connections) + 1}"
